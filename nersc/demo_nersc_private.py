...
if __name__ == "__main__":
    ...
    \n\t * Saving the recontruction results;\
    ...
    # Destination path to download and extract the NERSC data
    ...
    if dataset == 'fuelcell':
        center_of_rotation = 1283.0
    else:
        center_of_rotation = 1208.875
    sharpness = 1.0
    ...
    # Crop the bright ring artifacts around the boundary
    crop_ring_size = 50
    circular_mask = nersc_utils.create_circular_mask(recon.shape[0]-2*crop_ring_size, recon.shape[1]-2*crop_ring_size).astype(int)
    recon = recon[crop_ring_size:-crop_ring_size, crop_ring_size:-crop_ring_size, :] * circular_mask[:, :, np.newaxis]
    ...
    # Display the results
    mj.slice_viewer(recon, data_dicts=recon_dict, vmin = 0, vmax = 10, title=f'MBIR Reconstruction for {dataset} data')