...
def remove_stripe(sino, snr=3, large_filter_size=61, small_filter_size=21):
    ...
    Args:
        ...
        large_filter_size (int, optional): Window size of the median filter to remove large stripes
        small_filter_size (int, optional): Window size of the median filter to remove small-to-medium stripes
...
def remove_stripe_wavelet_fourier(sino, wavelet_filter_name="db5", sigma=2):
    ...
    Args:
        sino (jax array): 3D sinogram data with shape (num_views, num_det_rows, num_det_channels)
        wavelet_filter_name (str, optional): wavelet filter type.
