...
def run_pca(hsnt_data, dataset_name, ind_sets):
    ...
    for j, inds in enumerate(ind_sets):
        ...
        pct_explained = 100 * (1 - nrmse**2)
        recon_svd += hsnt_mean
        recon_svd = recon_svd.reshape(hsnt_data.shape)
        print('Pct variance for PCA recon with inds = {}: {:.4g}%'.format(inds, pct_explained))
        ...
        ax_img.imshow(image)
        ax_img.set_title(
            f"Component(s) {inds} explain {pct_explained:.4g}% of variance"
        )
        ax_img.axis("off")
...
def main():
    """
    Perform PCA and NNMF on a specified set of spatial/spectral Raman data in csv form.
    The data format is assumed to be:
        1. First row is calibration data (see below)
        2. Remaining rows have this format:
            a. First two columns specify x and y coordinates within a sample
            b. Remaining columns specify spectral intensity, one column per bin
        First row calibration has the first two columns empty, then blank scan intensities in each other column.

    The parameter ind_sets specifies which PCA/NNMF components to plot.

    """
    # Choose dataset
    # 'Nilotinib-P, 50X, 785, 1200_01.csv'
    # 'Nilotinib-AS, 50X, 785, 1200_01.csv'
    ...
    # The index sets should be disjoint and have union equal to 0, ..., N
    ...
    for j, inds in enumerate(ind_sets):
        ...
        rehydrated_data = subspace_data[:, :, cum_inds] @ subspace_basis[cum_inds]

        if dataset_type == 'transmission':
            rehydrated_data = np.exp(-rehydrated_data)  # Convert to transmission

        # mj.slice_viewer(hsnt_data, rehydrated_data, slice_axis=2,
        #                 title='Orig data (left), denoised data with inds {} (right)'.format(cum_inds))
        nrmse = np.linalg.norm(rehydrated_data - hsnt_data)**2 / np.linalg.norm(hsnt_data)**2
